...
from src.modelo.VO.ClaseOcupacionClienteVO import ClaseOcupacionClienteVO
from src.modelo.VO.ClaseSalaVO import ClaseSalaVO


class ClaseConsultasDaoJDBC(DaoJDBCBase):
    """
    DAO de consultas sobre la tabla clase.
    """

    # Busca clases por nombre 
    SQL_BUSCAR_CLASES = (
        "SELECT id_clase, nombre_actividad, dia_semana, hora_inicio, hora_fin, "
        "aforo_maximo, nivel_intensidad, calorias_estimadas "
        "FROM clase "
        "WHERE LOWER(nombre_actividad) LIKE ? "
        "ORDER BY nombre_actividad"
    )

    # Ocupación global de todas las clases 
    SQL_OCUPACION_CLASE = (
    ...
    # Clases de un entrenador con sala, horario  y capacidad actual/máxima
    SQL_CLASES_ENTRENADOR_TABLA = (
        "SELECT c.nombre_actividad, s.nombre AS sala, "
        ...
        "GROUP BY c.id_clase, c.nombre_actividad, s.nombre, c.hora_inicio, c.hora_fin, "
        "c.dia_semana, c.aforo_maximo "
        "ORDER BY FIELD(c.dia_semana,'lunes','martes','miercoles','jueves','viernes','sabado','domingo'), "
        "c.hora_inicio"
    )

    # Ocupación de las clases de un entrenador 
    SQL_OCUPACION = (
        "SELECT c.id_clase, c.nombre_actividad, COUNT(i.id_inscripcion) AS inscritos, "
        "c.aforo_maximo, CAST(COUNT(i.id_inscripcion)*100.0/c.aforo_maximo AS DECIMAL(5,2)) AS ocupacion "
        "FROM clase c "
        "LEFT JOIN inscripcion i ON c.id_clase = i.id_clase AND i.estado = 'inscrito' "
        "WHERE c.id_entrenador = ? "
        "GROUP BY c.id_clase, c.nombre_actividad, c.aforo_maximo "
    ...
    # Información de una clase junto con el nombre de su sala
    SQL_INFORMACION_SALA = (
        "SELECT c.nombre_actividad, s.nombre, c.dia_semana, c.hora_inicio, c.hora_fin, c.aforo_maximo "
        "FROM clase c "
        "INNER JOIN sala s ON c.id_sala = s.id_sala "
        "WHERE c.id_clase = ?"
    )

    # Datos completos de una clase por su ID
    SQL_BUSCAR_CLASE = (
        "SELECT id_clase, id_entrenador, id_sala, nombre_actividad, calorias_estimadas, "
        "dia_semana, hora_inicio, hora_fin, duracion, aforo_maximo, nivel_intensidad "
        "FROM clase "
        "WHERE id_clase = ?"
    )

    # Número de clases programadas para el día actual
    SQL_RECEPCION_CLASES_HOY = (
    ...
    # Número de clases de un entrenador programadas para hoy
    SQL_CLASES_HOY_ENTRENADOR = (
        "SELECT COUNT(*) FROM clase "
        "WHERE id_entrenador = ? "
    ...
    # Todas las clases con sala, inscritos y aforo 
    SQL_CLASES_OCUPACION_CLIENTE = """
        ...
        GROUP BY 
            c.id_clase, c.nombre_actividad, c.dia_semana,
            c.hora_inicio, c.hora_fin, s.nombre, c.aforo_maximo
        ORDER BY FIELD(c.dia_semana,'lunes','martes','miercoles','jueves','viernes','sabado','domingo'),
                 c.hora_inicio
    """
    ...
    def clases_ocupacion_cliente(self):
        ...
        filas = self.consultar(self.SQL_CLASES_OCUPACION_CLIENTE)
        return [ClaseOcupacionClienteVO(f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7])
                for f in filas]
    ...
    def buscar_clases(self, texto: str):
        """Busca clases cuyo nombre tenga el texto indicado 
    ...
    def ocupacion_clases(self):
        """Devuelve la ocupación de todas las clases
        como lista de OcupacionClaseVO, ordenadas de mayor a menor ocupación."""
    ...
    def clases_entrenador_tabla(self, id_entrenador):
        """Devuelve las clases de un entrenador con sala, horario 
        y capacidad actual/máxima como lista de ClaseEntrenadorVO."""
    ...
    def informacion_clase_con_sala(self, id_clase):
        ...
        if not datos:
            return None
        f = datos[0]
        return ClaseSalaVO(f[0], f[1], f[2], f[3], f[4], f[5] if len(f) > 5 else None)
    ...
    def recepcion_clases_hoy(self):
        """Devuelve el número total de clases programadas para el día actual.
        Usada en el panel de inicio del recepcionista."""
